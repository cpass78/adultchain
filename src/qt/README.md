This directory contains the AdultChainQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the AdultChain UI.
...
ClientModel is used by the main application `adultchaingui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/bitcoin/bitcoin/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a adultchain:... link from a browser).
...
* UI elements like AdultChainAmountField, which inherit from QWidget.
* `adultchainstrings.cpp`: automatically generated
* `adultchainunits.(h/cpp)`: BTC / mBTC / etc handling
* `callback.h`
...
* don't change `local/adultchain_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "adultchain-qt" under "Run", which is where you can also add command line arguments)
