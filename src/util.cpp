...
const char * const ADULTCHAIN_CONF_FILENAME = "adultchain.conf";
const char * const ADULTCHAIN_PID_FILENAME = "adultchaind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "adultchaind -foo=bar
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Adultchain";
#else
    // Unix
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", ADULTCHAIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", ADULTCHAIN_PID_FILENAME)));
}
